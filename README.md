...
This is generated by `make examples` (which runs `make scan` against the sample app). 
...
control, guardrail policy, scanner or stack component — and each edge is a markdown
link between them. It is a browsing aid; the scan pipeline reads the same files
...
| `examples/` | a sample report produced by `make examples` |
